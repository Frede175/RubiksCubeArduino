...
*   Sets which wires should control the motor.
*/
MCPStepper::MCPStepper(int number_of_steps, int motor_pin_1, int motor_pin_2, int motor_pin_3, int motor_pin_4)
{
	this->hasMCP = false;
	this->isPinModeSet = false;
...
*   Sets which wires should control the motor.
*/
MCPStepper::MCPStepper(int number_of_steps, int motor_pin_1, int motor_pin_2, int motor_pin_3, int motor_pin_4, int motor_pin_5)
{
	this->hasMCP = false;
	this->isPinModeSet = false;
...
*/
void MCPStepper::setSpeed(long whatSpeed)
{
...
*/
void MCPStepper::step(int steps_to_move)
{
...
*/
void MCPStepper::stepMotor(int thisStep)
{
	if (this->pin_count == 2) {
		switch (thisStep) {
		case 0:  // 01
			_digitalWrite(motor_pin_1, LOW);
			_digitalWrite(motor_pin_2, HIGH);
			break;
		case 1:  // 11
		...
		case 0:  // 1010
			_digitalWrite(motor_pin_1, HIGH);
			_digitalWrite(motor_pin_2, LOW);
			_digitalWrite(motor_pin_3, HIGH);
			_digitalWrite(motor_pin_4, LOW);
			break;
		case 1:  // 0110
			_digitalWrite(motor_pin_1, LOW);
			_digitalWrite(motor_pin_2, HIGH);
		...
		case 0:  // 01101
			...
			_digitalWrite(motor_pin_3, HIGH);
			_digitalWrite(motor_pin_4, LOW);
			_digitalWrite(motor_pin_5, HIGH);
			break;
		case 1:  // 01001
			_digitalWrite(motor_pin_1, LOW);
			_digitalWrite(motor_pin_2, HIGH);
...
void MCPStepper::_digitalWrite(uint8_t p, uint8_t d)
{
	if (this->hasMCP == true) {
		this->mcp.digitalWrite(p, d);
	}		
}

void MCPStepper::_pinMode(uint8_t p, uint8_t d)
{
	if (this->hasMCP == true) {
		this->mcp.pinMode(p, d);
		this->isPinModeSet = true;
	}
		
}

void MCPStepper::setMCP(Adafruit_MCP23017 mcp) {
	this->mcp = mcp;
	this->hasMCP = true;
	if (this->pin_count == 5) {

		_pinMode(this->motor_pin_1, OUTPUT);
		_pinMode(this->motor_pin_2, OUTPUT);
		_pinMode(this->motor_pin_3, OUTPUT);
		_pinMode(this->motor_pin_4, OUTPUT);
		_pinMode(this->motor_pin_5, OUTPUT);
	}
	else if (this->pin_count == 4) {
		_pinMode(this->motor_pin_1, OUTPUT);
		_pinMode(this->motor_pin_2, OUTPUT);
